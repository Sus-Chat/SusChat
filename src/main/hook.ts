...
async function loadOffsets(
...
): Promise<IOffsets | undefined> {
	const valuesFile = resolve(
		(process.env.LOCALAPPDATA || '') + 'Low',
		'Innersloth/Among Us/Unity/6b8b0d91-4a20-4a00-a3e4-4da4a883a5f0/Analytics/values',
	);
	let version = '';
	if (existsSync(valuesFile)) {
		try {
			...
			event.reply(
				'error',
				`Couldn't determine the Among Us version - ${e}. Try opening Among Us and then restarting Sus Chat.`,
			);
			return;
		}
	} else {
		event.reply(
			'error',
			"Couldn't determine the Among Us version - Unity analytics file doesn't exist. Try opening Among Us and then restarting Sus Chat.",
		);
		return;
	}
	...
	if (version === offsetStore.version) {
		...
		try {
			...
			if (e?.response?.status === 404) {
				event.reply(
					'error',
					`You are on an unsupported version of Among Us: ${version}.\n`,
				);
			} else {
				...
				event.reply(
					'error',
					`Please use another voice server. ${store.get(
						'serverURL',
					)} ${errorMessage}.`,
				);
			}
	...
	try {
		...
		if (!version) {
			event.reply(
				'error',
				"Couldn't determine the Among Us version. Try opening Among Us and then restarting Sus Chat.",
			);
			return;
		} else {
			store.set('offsets', {
		...
		event.reply(
			'error',
			`Couldn't parse the latest game offsets from the server: ${store.get(
				'serverURL',
			)}/${version}.yml.\n${e}`,
		);
		return;
